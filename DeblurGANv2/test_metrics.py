...
def get_args():
	...
	parser.add_argument("--config_path", type=str, default='config/config.cfg', help="path to the config file")
...
def get_gt_image(path):
	...
	# custom dataset
	img = cv2.cvtColor(cv2.imread(os.path.join(base, 'sharp_square', seq, filename)), cv2.COLOR_BGR2RGB)
	return img
...
def test_image(model, image_path):
	...
	print(gt_image.shape, result_image.shape)
...
if __name__ == '__main__':
	...
	# ----test
	# ----train
	filenames = sorted(glob.glob(args.img_folder + '/blured/test/*.jpg', recursive=True))
