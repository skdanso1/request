    ...
    def train(self):
        ...
        for epoch in range(0, config['num_epochs']):
            ...
            if self.metric_counter.update_best_model():
                ...
                }, 'ckpt/best_{}.h5'.format(self.config['experiment_desc']))
            torch.save({
                'model': self.netG.state_dict()
            }, 'ckpt/last_{}.h5'.format(self.config['experiment_desc']))
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="DeblurGANv2 trainer")

    parser.add_argument("--config_path", type=str, default='config/config.yaml', help="path to the config file")
    args = parser.parse_args()
    assert os.path.exists(args.config_path), 'Config file does not exists, please check it!'
