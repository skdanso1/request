...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:
            ...
            giou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, GIoU=False, DIoU=True, CIoU=False)  # giou(prediction, target)
...
def plot_results(start=0, stop=0, bucket='', id=()):  # from utils.utils import *; plot_results()
    ...
    fig, ax = plt.subplots(2, 5, figsize=(18, 10), tight_layout=True)
    ax = ax.ravel()
    ...
    if bucket:
        ...
        files = glob.glob('results_*.txt') + glob.glob('../../Downloads/results*.txt')
        ...
        try:
            ...
            for i in range(10):
                ...
                if i in [0, 1, 2, 5, 6, 7]:
                    y[y == 0] = np.nan  # dont show zero loss values
                    # y /= y[0]  # normalize
                ax[i].plot(x, y, marker='.', label=Path(f).stem[8:], linewidth=1, markersize=3)
                ax[i].set_title(s[i])
